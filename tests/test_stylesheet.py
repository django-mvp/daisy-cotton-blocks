...
— needs the Node toolchain and so runs in the Stylesheet workflow.
"""
...
def test_both_stylesheets_are_discoverable() -> None:
...
def test_stylesheets_are_not_empty(
    mvp_classes: set[str], bits_classes: set[str]
) -> None:
...
def test_stylesheet_carries_no_theme_or_preflight() -> None:
    """The supplement must not restate what django-mvp defines.

    Re-emitting the theme layer would put a second `:root` block on the page and
    let this package silently win a cascade it has no business entering; a
    second preflight would reset elements django-mvp has already styled.
    """
...
def test_every_class_used_in_a_template_actually_exists(
    ...
    django-mvp is stable. Losing a class because django-mvp stopped emitting it
    is a supported outcome: this package does not work without django-mvp's
    stylesheet and has never pretended otherwise.
    """
