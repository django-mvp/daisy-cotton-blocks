...
def test_component_directory_is_where_cotton_looks_for_it() -> None:
    ...
    name is not a free choice: renaming it breaks every component tag at once,
    and does so silently — a missing component renders as empty output rather
    than raising.
    """
    components = Path(mvp_bits.__file__).parent / "templates" / "cotton" / "mvp_bits"
